...
def build_history(raw_history: Any, max_messages: int) -> list[types.Content]:
    history = []
...
def parse_chat_request(app: Flask, payload: dict[str, Any]) -> tuple[str, str, list[types.Content]]:
    """Validate a chat payload and build (persona, humor, history) or raise ChatRequestError."""
    ...
    if not message and not raw_attachments:
        raise ChatRequestError("Please enter a message.", 400)
    if persona not in PERSONAS:
        raise ChatRequestError("Unknown assistant persona.", 400)
    if humor not in HUMOR_LEVELS:
        raise ChatRequestError("Unknown humor level.", 400)
    if not isinstance(raw_history, list):
        raise ChatRequestError("Conversation history must be a list.", 400)
    if not app.config["GEMINI_API_KEY"]:
        raise ChatRequestError("GEMINI_API_KEY is not configured on the server.", 503)

    try:
        files = attachment_parts(raw_attachments)
    except ValueError as exc:
        raise ChatRequestError(str(exc), 400) from exc

    history = build_history(raw_history, app.config["MAX_HISTORY_MESSAGES"])
